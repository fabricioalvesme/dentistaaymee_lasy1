...
const Notifications = () => {
  ...
  const loadPatientInfo = async (remindersList: Reminder[]) => {
    try {
      ...
      // Criar mapa de ID -> informações do paciente
  ...
  // Filtrar lembretes com base na tab ativa
  const filteredReminders = () => {
    switch (activeTab) {
      case 'upcoming':
        return reminders.filter(r => new Date(r.target_date) >= new Date() && !r.sent);
      case 'past':
        return reminders.filter(r => new Date(r.target_date) < new Date() || r.sent);
      case 'birthdays':
        // Mostrar aniversários virtuais
        return [];
      default:
        return reminders;
    }
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Notificações que precisam da sua atenção
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
                      ...
                      <div key={notification.id} className="bg-gray-50 p-4 rounded-lg">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-gray-600 mt-1">
                              Retorno de {patient?.nome || 'Paciente'} agendado para {formatDate(reminder.target_date)}
                            </p>
                          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Todos os lembretes cadastrados no sistema
            </CardDescription>
          </CardHeader>
          <CardContent>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <h3 className="font-medium">
                                {birthday.dias_ate_aniversario === 0 ? 'Aniversário hoje' : 'Aniversário amanhã'}
                              </h3>
                              <Badge variant="outline" className="bg-pink-50 text-pink-600 border-pink-100">
                                {birthday.dias_ate_aniversario === 0 ? 'Hoje' : 'Amanhã'}
                              </Badge>
                            </div>
                            
                            <p className="text-gray-600 mt-1">
                              {birthday.nome} - {formatDate(birthday.data_nascimento)}
                            </p>
                          </div>
...
// Componente de tabela de lembretes
          ...
          {reminders.map(reminder => {
            ...
            return (
              <TableRow key={reminder.id}>
                <TableCell>
                  {patient?.nome || 'Carregando...'}
                </TableCell>
                <TableCell>
                  <Badge variant="outline" className="bg-blue-50 text-blue-600 border-blue-100">
                    {reminder.type === 'return' ? 'Retorno' : 'Outro'}
                  </Badge>
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-1">
                    <Clock className="h-3 w-3 text-gray-400" />
                    <span>{formatDate(reminder.target_date)}</span>
                  </div>
                </TableCell>
                <TableCell>
                  {reminder.sent ? (
